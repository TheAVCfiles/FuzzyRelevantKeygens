...
- `pnpm run audit:release --expected-commit "$(git rev-parse HEAD)" --expected-tree "$(git rev-parse HEAD^{tree})"` — local, offline exact-commit gate required before any future publish; it checks cleanliness, identity, generated clients, and lockfile freshness and never publishes
- Configure exactly one Gemini transport for the ADK and direct GenAI paths: Developer API with one matching `GEMINI_API_KEY`, `GOOGLE_GENAI_API_KEY`, or `GOOGLE_API_KEY`; or Vertex AI with `GOOGLE_GENAI_USE_VERTEXAI=true` (or `GOOGLE_GENAI_USE_ENTERPRISE=true`), `GOOGLE_CLOUD_PROJECT`, `GOOGLE_CLOUD_LOCATION`, and Application Default Credentials. Mixed, incomplete, or custom-endpoint settings fail closed.
...
- ADK/direct execution evidence may include only framework, provider, model, execution ID, declared tools, latency, activity, status, and the safe API/auth transport descriptor. Never include prompts, model output, credentials, project IDs, or locations.
...
- Commit intended release changes before running the exact-commit audit; a dirty checkout always fails.
