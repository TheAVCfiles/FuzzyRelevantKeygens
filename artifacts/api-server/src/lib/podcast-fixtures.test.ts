...
function reverseObjectKeys(value: unknown): unknown {
  ...
  return Object.entries(value as Record<string, unknown>)
    .reverse()
    .reduce<Record<string, unknown>>((reordered, [key, item]) => {
      reordered[key] = reverseObjectKeys(item);
      return reordered;
    }, {});
}
...
test("synthetic approved cut produces a canonical, private-safe Cut Key only after every gate", { concurrency: false }, async () => {
  ...
  const sources = sourceIds.map((id, index) => ({
    id,
    url: index === 0
      ? "https://example.test/cut/0?utm_source=private&keep=1#private-marker"
      : `https://example.test/cut/${index}`,
    title: `Cut source ${index}`, retrieved_at: "2026-01-01T00:00:00.000Z",
    snippet: "", source_type: "test", classification: "source_backed" as const,
  ...
  assert.deepEqual(Object.keys(firstManifest).sort(), [
    "approval_records", "audio_sha256", "audio_url", "claim_support", "clip_id",
    "execution_envelope", "format_disclosure", "generated_at", "integrity_disclaimer",
    "key", "manifest_sha256", "manifest_version", "production", "source_ids",
    "source_metadata", "transcript", "transcript_sha256", "voice_disclosure",
  ]);
  assert.equal(firstManifest.manifest_version, 2);
  assert.deepEqual(firstManifest.approval_records?.map((record) => record.record_type), [
    "DEVELOPMENT_VALIDATED",
    "BRIEF_APPROVED",
    "SCRIPT_APPROVED",
    "AUDIO_RENDER_AUTHORIZED",
  ]);
  assert.ok(firstManifest.approval_records?.every((record) => /^[a-f0-9]{64}$/.test(record.subject_sha256)));
  assert.ok(firstManifest.approval_records?.every((record) => /^[a-f0-9]{64}$/.test(record.record_sha256)));
  assert.ok(firstManifest.approval_records?.every((record) => !record.reviewer_reference.includes("reviewer")));
  assert.equal(firstManifest.source_metadata?.length, 3);
  assert.ok(firstManifest.source_metadata?.every((source) => !("snippet" in source) && !("consent_reference" in source)));
  assert.equal(firstManifest.source_metadata?.[0]?.url, "https://example.test/cut/0?keep=1");
  assert.equal(firstManifest.claim_support?.length, created.script.sections.length);
  assert.deepEqual(firstManifest.claim_support?.map((claim) => claim.claim_text), created.script.sections.map((section) => section.script));
  ...
  assert.deepEqual(firstManifest.production, { synthetic: true, provider: "Google Gemini", model: "gemini-2.5-flash-preview-tts" });
  ...
  assert.equal(
    commitGeneratedPodcastAudio(secondDecision.script, Buffer.alloc(52)).kind,
    "superseded",
    "a prior audio authorization must not authorize a new render revision",
  );
  recordPodcastDecision("audio", created.script.id, "approve", "second-audio-reviewer");
  ...
  const reorderedManifest = {
    ...
    execution_envelope: reverseObjectKeys(secondManifest.execution_envelope),
    approval_records: reverseObjectKeys(secondManifest.approval_records),
    claim_support: reverseObjectKeys(secondManifest.claim_support),
    source_metadata: reverseObjectKeys(secondManifest.source_metadata),
    manifest_version: secondManifest.manifest_version,
  };
...
test("judge manifest returns an active canonical non-synthetic cut without private state", { concurrency: false }, async () => {
  const transcript = "FRONT ROW: What changed? BACKSTAGE: Here is the sourced context.";
  const sections = [{
    segment: "evidence",
    script: "Here is the sourced context.",
  ...
  const provenance = [
    {
      ...
      url: "https://example.test/public-source-1",
      label: "Approved public source 1",
      retrieved_at: "2026-09-09T00:00:00.000Z",
    },
    {
      source_id: "public-source-2",
      url: "https://example.test/public-source-2",
      label: "Approved public source 2",
      retrieved_at: "2026-09-09T00:00:00.000Z",
    },
  ];
  const releaseKit = {
    id: "release-kit-judge-live-script",
    script_id: "judge-live-script",
    status: "staged" as const,
    title_options: ["Judge sample"],
    episode_description: "A bounded, sourced sample.",
    chapters: [{ label: "Evidence", timing: "00:00–00:20", purpose: "Show support." }],
    host_notes: ["Keep uncertainty visible."],
    promotion_copy: [{ channel: "show notes", copy: "A bounded sample." }],
    accessibility_notes: ["Publish the transcript."],
    provenance_summary: "Two public sources support this sample.",
    audio_status: "generated" as const,
    publishing_status: "blocked_until_final_approval" as const,
    next_reviewer: "Publishing approver",
  };
  const judgeScript = {
    id: "judge-live-script",
    brief_id: "judge-live-brief",
    run_id: "judge-live-run",
    attestation_id: "judge-live-attestation",
    status: "approved" as const,
    title: "Judge sample",
    sections,
    provenance,
    safety_note: "Keep claims bounded.",
    review_note: "Approved for this evidence render.",
    audio_status: "generated" as const,
    compatibility_normalized: false,
    workspace_revision: "judge-live-revision",
    ...
    audio_clip: {
      ...
      audio_url: "/api/podcast/audio/clip-judge-live/stream",
  ...
  const scriptSubjectSha256 = canonicalSha256({
    id: judgeScript.id,
    brief_id: judgeScript.brief_id,
    run_id: judgeScript.run_id,
    attestation_id: judgeScript.attestation_id,
    status: judgeScript.status,
    title: judgeScript.title,
    sections,
    provenance,
  });
  const renderIdentity = JSON.stringify(canonicalJsonValue({
    workspace_revision: judgeScript.workspace_revision,
    brief_id: judgeScript.brief_id,
    title: judgeScript.title,
    status: judgeScript.status,
    audio_status: "ready_to_generate",
    ...
    source_ids: provenance.map((source) => source.source_id),
    release_kit_status: releaseKit.status,
    release_kit_audio_status: "ready_to_generate",
  ...
  const approvalRecord = (
    ...
    const record = {
      ...
      artifact_id: "judge-live-script",
      reviewer_reference: createHash("sha256").update(`reviewer-${suffix}`).digest("hex"),
      decided_at: `2026-09-09T00:0${suffix}:00.000Z`,
  ...
  const canonicalPayload = {
    ...
    source_metadata: [
      {
        ...
        aggregate_summary: "A bounded public pattern summary.",
        evidence_gaps: ["Intent remains unresolved."],
        what_it_supports: "A public format pattern.",
      ...
      },
      {
        ...
        classification: "source_backed" as const,
        policy_reference: "podcast-current-context-policy-v1",
        aggregate_summary: "A second bounded public pattern summary.",
        evidence_gaps: ["Representativeness remains unresolved."],
        what_it_supports: "A second public format pattern.",
        what_remains_uncertain: "The source is not representative of all viewers.",
      },
    ],
    claim_support: [{
      claim_id: "claim-01",
    ...
    approval_records: [
      approvalRecord("SCRIPT_APPROVED", "script", "1", scriptSubjectSha256),
      approvalRecord(
        "AUDIO_RENDER_AUTHORIZED",
        "audio",
        "2",
        canonicalSha256({ render_identity: renderIdentity }),
      ),
    ],
    execution_envelope: [{
      ...
      execution_id: "judge-adk-execution",
      tools: ["googleSearch"],
      latency_ms: 42,
  ...
  const manifestSha256 = canonicalSha256(canonicalPayload);
  ...
  assert.equal(publicManifest.manifest_version, 2);
  assert.ok(publicManifest.execution_envelope?.some((execution) => execution.framework === "Google ADK (@google/adk)"));
  assert.ok(publicManifest.approval_records?.some((record) => record.record_type === "SCRIPT_APPROVED"));
  assert.ok(publicManifest.approval_records?.some((record) => record.record_type === "AUDIO_RENDER_AUTHORIZED"));
  assert.equal("approval_receipts" in publicManifest, false);
  assert.equal("private_attestation" in publicManifest, false);

  const placeholderSourcePayload = {
    ...canonicalPayload,
    source_metadata: canonicalPayload.source_metadata.map((source, index) => ({
      ...source,
      title: `Approved public-web result ${index + 1}`,
      url: `https://vertexaisearch.cloud.google.com/grounding-api-redirect/source-${index + 1}`,
      aggregate_summary: "Provider-linked source contributed a topic-level signal.",
      what_it_supports: "A topic-level signal about audience preferences.",
    })),
  };
  const placeholderSourceSha256 = canonicalSha256(placeholderSourcePayload);
  assert.equal(rehydratePodcastState({
    briefs: [],
    scripts: [judgeScript],
    cutKeys: [{
      key: `cut-${placeholderSourceSha256}`,
      manifest_sha256: placeholderSourceSha256,
      audio_url: `/api/podcast/cut-keys/cut-${placeholderSourceSha256}/audio`,
      ...placeholderSourcePayload,
    }],
    currentBriefId: null,
    currentScriptId: judgeScript.id,
  }), true);
  assert.equal(await getPublicPodcastJudgeManifest(), null);

  const unsupportedAudiencePayload = {
    ...canonicalPayload,
    claim_support: canonicalPayload.claim_support.map((claim) => ({
      ...claim,
      claim_text: "Fans clearly preferred the tribute format.",
    })),
  };
  const unsupportedAudienceSha256 = canonicalSha256(unsupportedAudiencePayload);
  assert.equal(rehydratePodcastState({
    briefs: [],
    scripts: [judgeScript],
    cutKeys: [{
      key: `cut-${unsupportedAudienceSha256}`,
      manifest_sha256: unsupportedAudienceSha256,
      audio_url: `/api/podcast/cut-keys/cut-${unsupportedAudienceSha256}/audio`,
      ...unsupportedAudiencePayload,
    }],
    currentBriefId: null,
    currentScriptId: judgeScript.id,
  }), true);
  ...
  const failedExecutionPayload = {
    ...canonicalPayload,
    execution_envelope: canonicalPayload.execution_envelope.map((execution) => ({
      ...execution,
      status: "failed" as const,
    })),
  };
  const failedExecutionSha256 = canonicalSha256(failedExecutionPayload);
  assert.equal(rehydratePodcastState({
    briefs: [],
    scripts: [judgeScript],
    cutKeys: [{
      key: `cut-${failedExecutionSha256}`,
      manifest_sha256: failedExecutionSha256,
      audio_url: `/api/podcast/cut-keys/cut-${failedExecutionSha256}/audio`,
  ...
  const swappedApprovalPayload = {
    ...canonicalPayload,
    approval_records: canonicalPayload.approval_records.map((record) => {
      if (record.record_type !== "AUDIO_RENDER_AUTHORIZED") return record;
      const swapped = { ...record, artifact_id: "different-script" };
      const { record_sha256: _ignored, ...canonical } = swapped;
      return { ...canonical, record_sha256: canonicalSha256(canonical) };
    }),
  };
  const swappedApprovalSha256 = canonicalSha256(swappedApprovalPayload);
  assert.equal(rehydratePodcastState({
    briefs: [],
    scripts: [judgeScript],
    cutKeys: [{
      key: `cut-${swappedApprovalSha256}`,
      manifest_sha256: swappedApprovalSha256,
      audio_url: `/api/podcast/cut-keys/cut-${swappedApprovalSha256}/audio`,
      ...swappedApprovalPayload,
    }],
    currentBriefId: null,
    currentScriptId: judgeScript.id,
  }), true);
...
test("Google grounding redirects resolve only to sanitized public canonical URLs", async () => {
  const redirect = "https://vertexaisearch.cloud.google.com/grounding-api-redirect/opaque";
  const request = async () => new Response(null, {
    status: 302,
    headers: {
      location: "https://www.reddit.com/r/Nirvana/comments/example?utm_source=google&keep=1#fragment",
    },
  });
  assert.equal(
    await resolveGroundingSourceUrl(redirect, request as typeof fetch),
    "https://www.reddit.com/r/Nirvana/comments/example?keep=1",
  );

  const unsafeRequest = async () => new Response(null, {
    status: 302,
    headers: { location: "http://127.0.0.1/private" },
  });
  assert.equal(await resolveGroundingSourceUrl(redirect, unsafeRequest as typeof fetch), null);
  assert.equal(await resolveGroundingSourceUrl("javascript:alert(1)", request as typeof fetch), null);
});
